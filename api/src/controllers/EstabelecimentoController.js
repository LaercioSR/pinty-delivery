...
module.exports = {
  async index(request, response) {
    ...
    const estabelecimentosSelerializado = estabelecimentos.map(
      (estabelecimento) => {
        const linkImagem = estabelecimento.imagem
          ? "/uploads/estabelecimento_images/" + estabelecimento.imagem
          : "/uploads/default.png";
        return {
  ...
  async store(request, response) {
    const { nome, sobre, endereco, imagem, whatsapp, email, categoria_id } =
      request.body;
    let imagem_nome = "";
    ...
    if (imagem) {
      ...
      FileSystem.writeFile(
        "public/uploads/estabelecimento_images/" + imagem_nome,
        base64_data,
        "base64",
        function (err) {
          console.log(err);
